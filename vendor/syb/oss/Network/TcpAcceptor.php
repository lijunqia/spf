<?php
//**********************************************************
// File name: TcpAcceptor.class.php
// Class name: TcpAcceptor
// Create date: 2011/3/30
// Update date: 2011/3/30
// Author: parkerzhu
// Description: Tcp接收请求包装类
//**********************************************************
 ...
 *
 */
class TcpConnector extends Socket
{
     ...
     * @return 0: 成功 -1: 失败
     */
    public function Accept(TcpStream &$tcpStream, $sAddr, $iPort, $iTimeout = -1, $iSendBufferSize = -1)
    {
    ...
    public function NonBlockConnect(TcpStream &$tcpStream, $sAddr, $iPort, $iTimeout, $iSendBufferSize)
    {
        ...
        if ($iSendBufferSize >= 0) {
            if ($tcpStream->SetSockOption(SOL_SOCK, SO_SNDBUF, $iSendBufferSize) == -1) {
                return -1;
            }
        }
