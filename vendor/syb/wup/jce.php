...
abstract class data_type
{
	...
	public function _skip_struct(&$stream)
	{
		...
		if ($this_tag != 0) throw new JCEException(__LINE__, TAG_NOT_MATCH);
		if ($struct_type != 'c_struct_end') throw new JCEException(__LINE__, TYPE_NOT_MATCH);
	}
...
class c_char extends data_type
{
	...
	public function __set($name, $val)
	{
		...
		if ($name == 'val') {
			...
			if (is_int($val)) {
				if ($val > 127 || $val < -128) throw new JCEException(__LINE__, OUTOF_RANGE);
				$this->val = chr($val);
			} else if (is_null($val)) {
				$this->val = chr(0);
				...
				if (strlen($val) != 1) throw new JCEException(__LINE__, LEN_NOT_MATCH);
				$this->val = $val;
			}
		} else {
	...
	//是wup支持基本类型
	public function is_base_datatype()
	{
		return TRUE;
	}
	...
	public function read(&$stream, $tag, $isRequire = TRUE)
	{
		...
		if ($type != 'c_zero' && $type != 'c_char') throw new JCEException(__LINE__, TYPE_NOT_MATCH);
...
class c_int extends data_type
{
	...
	public function read(&$stream, $tag, $isRequire = TRUE)
	{
		...
		if ($type != 'c_zero' && $type != 'c_char' && $type != 'c_short' && $type != 'c_int') throw new JCEException(__LINE__, TYPE_NOT_MATCH);
		jce_header::_unpack_header($stream, $type, $this_tag);
		$this->_unpack_base($stream, $type, $val);
		if (is_string($val)) {
			//字符串到int的转换
...
class c_int64 extends data_type
{
	...
	public function read(&$stream, $tag, $isRequire = TRUE)
	{
		...
		if ($type != 'c_zero' && $type != 'c_char' && $type != 'c_short' && $type != 'c_int' && $type != 'c_int64') throw new JCEException(__LINE__, TYPE_NOT_MATCH);
		jce_header::_unpack_header($stream, $type, $this_tag);
		$this->_unpack_base($stream, $type, $val);
		if ($type == 'c_char') {
			$this->val = ord($val);
...
class c_vector extends data_type
{
	...
	public function __construct($proto)
	{
		...
		if (is_int($proto)) {
			...
			$this->class_name = _get_class($proto);
			$this->proto_type = clone $proto;
		}
	...
	{
		if ($this->class_name == 'c_char') return strlen($this->vector);
	...
	public function read(&$stream, $tag, $isRequire = TRUE)
	{
		...
		//存放char的simpl list和存放其他元素的list
		if ($type != 'c_simple_list' && $type != 'c_list') throw new JCEException(__LINE__, TYPE_NOT_MATCH);
		jce_header::_unpack_header($stream, $type, $this_tag);
		$this->vector = array();
		if ($type == 'c_simple_list') {
			//here ,unpack c_char tag
...
class c_map extends data_type
{
	...
	public function __construct($key_proto, $val_proto)
	{
		...
		if (is_int($key_proto)) {
			...
			$this->key_class = _get_class($key_proto);
			$this->key_proto = clone $key_proto;
		}
		if (is_int($val_proto)) {
			$val_proto = new c_int;
			...
			$this->val_class = _get_class($val_proto);
			$this->val_proto = clone $val_proto;
		}
	}
	...
	public function get_class_name()
	{
		$key_class = __NAMESPACE__.'\\'.$this->key_class;
		$val_class = __NAMESPACE__.'\\'.$this->val_class;
		$key_type = new $key_class;
		$val_type = new $val_class;
	...
	public function get_val($key)
	{
		...
		$class = __NAMESPACE__.'\\'.$this->val_class;
		return new $class;
	}
	...
	public function read(&$stream, $tag, $isRequire = TRUE)
	{
		...
		if ($type != 'c_map') throw new JCEException($type.'(line:'.__LINE__.')', TYPE_NOT_MATCH);
		jce_header::_unpack_header($stream, $type, $this_tag);
		$this->map = array();
		$len = new c_int;
		$len->read($stream, 0);
	...
	public function __set($name, $val)
	{
		foreach ($this as $item_key => $item) {
			if ($item_key == $name) {
				...
				if (!is_object($val)) {
					if ($this->$name->is_base_datatype() == TRUE) $this->$name->val = $val;
				} else {
					if (_get_class($val) != _get_class($this->$name)) throw new JCEException(__LINE__, TYPE_NOT_MATCH);
					$this->$name = $val;
				}
				return;
			}
