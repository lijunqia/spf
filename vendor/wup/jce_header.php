...
class jce_header
{
	...
	{
		//$type必须是表示类型名字的字符串
		if (!is_string($type)) throw new JCEException(__LINE__, HEADER_TYPE_ERROR);
		//tag必须是数字
		if (!is_int($tag)) throw new JCEException(__LINE__, HEADER_TAG_ERROR);
		//检查type是否合法。除了c_char 之外的不合法类型名字都会type中取的0值
		if ($type != 'c_char' && self::$type[ $type ] == 0) throw new JCEException(__LINE__, HEADER_TYPE_UNKNOWN);
		//tag大于15 pack方式不一样
		if ($tag >= 15) {
			$first_tag = 15;
	...
	static function _peek_header($stream, &$type, &$tag, $isRequire = TRUE)
	{
		...
		$tag = ord($header) >> 4;
		if ($tag == 15) {
			if ($len < 2) throw new JCEException(__LINE__, STREAM_LEN_ERROR);
			$tag = $stream{1};
			$tag = ord($tag);
		}
	}
